...
if ( !class_exists("PlulzFacebookApiException") )
{
    /**
     ...
     */
    class PlulzFacebookApiException extends Exception
    {
      /**
...
if ( !class_exists("PlulzBaseFacebook") )
{
    /**
     ...
     * @author Naitik Shah <naitik@facebook.com>
     */
    abstract class PlulzBaseFacebook extends PlulzObjectAbstract
    {
        /**
        * Version.
        */
        ...
        *
        * The parameters:
        * - ok_session: the URL to go to if a _session is found
        ...
        * The cookie is not set by the PlulzBaseFacebook class, but
        ...
        */
        protected function getUserFromAccessToken() {
            try {
              ...
              return $user_info['id'];
            } catch (PlulzFacebookApiException $e) {
              return 0;
            }
        }
        ...
        */
        protected function makeRequest($url, $params, $ch=null) {
            ...
            if ($result === false) {
              $e = new PlulzFacebookApiException(array(
        ...
        */
        protected function throwAPIException($result) {
            $e = new PlulzFacebookApiException($result);
            switch ($e->getType()) {
              // OAuth 2.0 Draft 00 style
              case 'OAuthException':
                // OAuth 2.0 Draft 10 style
        ...
        */
        ...
        /**
        * Get the data for $key, persisted by PlulzBaseFacebook::setPersistentData()
        *
...
{
    abstract class PlulzFacebookAbstract extends PlulzBaseFacebook
    {
        /**
        ...
        /**
         * Stores all the options inputed
         * @var array
         */
        protected $_options;
        ...
        public function __construct()
        {
            ...
            $this->appId    =   isset($this->_options['app'])    ?   $this->_options['app'] : '';
            $this->secret   =   isset($this->_options['secret']) ?   $this->_options['secret'] : '';
            $this->appLanguage = isset($this->_options['language']) ?   $this->_options['language'] : 'en_US';
        ...
        public function init()
        {
            ...
            $this->_likeOptions = array(
                'send'          =>  'send',      // checkbox
                'layout'        =>  array('standard' => 'standard','button_count' => 'button_count', 'box_count' => 'box_count'), // select
                'action'        =>  array('like' => 'like', 'recommend' => 'recommend'), // select
                'width'         =>  '370px',            //text
                'font'          =>  $this->getFonts(),  // select
                'colorscheme'   =>  $this->getColorScheme() // select
            );

            // #ref https://developers.facebook.com/docs/reference/plugins/comments/
            ...
            // #ref https://developers.facebook.com/docs/reference/plugins/activity/
            ...
            // #ref https://developers.facebook.com/docs/reference/plugins/live-stream/
            $this->_liveStreamOptions = array(
                'width'             =>  '400px', //text
                'height'            =>  '500px', //text
                'xid'               =>  '', // text
                'url'               =>  '', // text maybe use the blog url as default
                'postFriends'       =>  false
            );
            ...
            $output = "
                ...
                <script>
                  ...
                  (function(d){
                     ...
                     js.src = '//connect.facebook.net/{$this->appLanguage}/all.js';
        ...
        public function addOpenGraph()
		{
            ...
            else
            {
                ...
                // Lets define the title for the page
                if ( is_category() || is_tag() )
                {
                    $queriedObject = get_queried_object('name');

                    $postTitle = $queriedObject->name;
                }
                else
                    $postTitle = $siteName;

                $url    = PlulzTools::getCurrentPageUrl();
        ...
        public function socialLike( $newOptions = '' )
        {
            ...
            // If there is some override args lets replace it now, used mainly for shortcodes
            if ( !empty($newOptions) )
